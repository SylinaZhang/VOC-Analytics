...
function getMetric() {
    ...
    console.log("---------获取指标----------")
    const metric_data = JSON.parse(JSON.parse(localStorage.getItem('metrics')))
    console.log(metric_data)
    if (metric_data != null) {
        // 指标统计
        analysisMetric.value = metric_data
        termPercentage.value = metric_data.term_percentage
        tagTotalCount.value = metric_data.tag_total_count
        sentimentPercentages.value = metric_data.sentiment_percentages
        aspectItemCounts.value = metric_data.aspect_item_counts
        // 正负标签比
        pqchartData.value = setpqChartData(metric_data.tag_total_count)
        pqchartOptions.value = setpqChartOptions()
        // 四类标签数
        pqBarData.value  = setpqBarData(metric_data.tag_total_count)
        // 动机
        const exLabel = []
        const exdata = []
        metric_data.term_percentage.expectations.forEach(obj => {
            const key = obj[0];
        ...
        <div class="col-span-12 lg:col-span-6 xl:col-span-3">
            <div class="card mb-0">
                <div class="flex justify-between mb-4">
                    <div>
                        <span class="block text-muted-color font-medium mb-4">Total number of Tags</span>
